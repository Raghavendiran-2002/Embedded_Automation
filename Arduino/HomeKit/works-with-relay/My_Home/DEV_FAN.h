struct DEV_FAN : Service::Fan {               // First we create a derived class from the HomeSpan LightBulb Service
  ...
  boolean update(){            
    ...
    digitalWrite(fanPin,fanstatus);        // use a standard Arduino function to turn on/off ledPin based on the return of a call to power->getNewVal() (see below for more info)
